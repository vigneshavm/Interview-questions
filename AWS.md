...
| **Messaging Patterns**     | - **[Messaging Patterns – Key Points](#messaging-patterns--key-points)**                                                                                                                    | **Advanced Topics**            | - **[Invocation & Event Handling](#2-lambda-invocation--event-handling)**<br>- **[Architecture & Workflow Design](#3-architecture--workflow-design)**<br>- **[CI/CD & Deployment](#cicd-and-devops-azure-devops--nodejs)**<br>- **[Monitoring & Cost](#monitoring-lambda-functions)** | **Limits**                   | - **[Max Execution Time](#maximum-execution-time-of-an-aws-lambda-function)**<br>- **[Max Package Size](#maximum-deployment-package-size)**                                                |
...
| **Data Storage Structure in S3**      | S3 stores data as **objects** within **buckets**. Each object consists of **data**, **metadata**, and a **unique key**.                                                                                                  |
| **Comparison: S3 vs EBS**             | **S3** is for **object storage**, ideal for static files, backups, media. **EBS** is for **block storage**, used with **EC2** as a persistent disk.                                                                      |
| **S3 Storage Classes**                | - **Standard**: Frequently accessed data<br>- **Standard-IA/One Zone-IA**: Infrequent access<br>- **Glacier & Glacier Deep Archive**: Long-term archive<br>- **Intelligent-Tiering**: Auto-tiering for cost optimization |
| **Security and Access Control in S3** | S3 supports **IAM policies**, **bucket policies**, and **ACLs**. Data can be encrypted with **SSE-S3**, **SSE-KMS**, or **client-side encryption**.                                                                      |
| **Object Versioning**                 | **Versioning** allows storing **multiple versions** of an object to protect against overwrites or deletions.                                                                                                             |
| **Consistency Model in S3**           | S3 offers **strong read-after-write consistency** for all objects, ensuring immediate visibility of changes.                                                                                                             |
| **Using Presigned URLs for Access**   | **Presigned URLs** allow **temporary, secure access** to S3 objects without sharing credentials. Useful for controlled uploads/downloads.                                                                                |
| **Hosting Static Websites on S3**     | Enable **static website hosting**, define **index/error pages**, and configure **public read access** via bucket policy or ACL.                                                                                          |
| **Enforcing Server-Side Encryption**  | Apply a **bucket policy** to **deny uploads** without the **`x-amz-server-side-encryption`** header to enforce encryption.                                                                                               |
| **Multipart Upload Handling**         | For large files, **multipart upload** splits the file into parts, uploads them in **parallel**, and merges — optimizing **speed** and **reliability**.                                                                   |
| **Using S3 Transfer Acceleration**    | Uses **CloudFront edge locations** to accelerate uploads/downloads, especially for **geographically dispersed users**.                                                                                                   |
| **Cost Optimization Techniques**      | Use **lifecycle policies**, select optimal **storage classes**, clean up **incomplete uploads**, and enable **Intelligent-Tiering**.                                                                                     |
| **Bucket Privacy and Access Control** | A **private bucket** restricts access using **IAM**/**ACL**. A **public bucket** allows open access and must be used cautiously.                                                                                         |
| **Monitoring and Auditing Access**    | Enable **S3 access logs** or use **AWS CloudTrail** to monitor object-level and API-level activity for auditing and compliance.                                                                                          |
| **Handling Media Uploads in a Scalable App**  | I use **S3 for storing uploaded media** (images, videos) due to its **durability and scalability**. For secure client uploads, I generate **pre-signed URLs** with a **short expiry** and limit content type and size. |
| **Cost Optimization for Archived Data**       | For large volumes of rarely accessed data like logs, I implement **lifecycle policies** to transition data from **Standard to Glacier** or **Deep Archive**, significantly reducing storage cost.                      |
| **Implementing Static Website Hosting**       | I configure **S3 static website hosting**, set the **index.html and error.html**, make the bucket **public-read** using a bucket policy, and often use **CloudFront** for performance and HTTPS support.               |
| **Large File Uploads with Unstable Networks** | I use **multipart uploads**, allowing parallel upload of file chunks. If a network fails midway, only incomplete parts are retried, improving **resilience and efficiency**.                                           |
| **Cross-Region Backup Strategy**              | To ensure **disaster recovery**, I enable **Cross-Region Replication (CRR)** to replicate S3 objects to another AWS region automatically, using bucket policies and versioning.                                        |
| **Controlling Direct User Access to S3**      | Instead of exposing S3 directly, I serve files via **CloudFront** with **signed URLs or cookies**, restricting access and enabling geo-blocking and caching.                                                           |
| **Preventing Accidental Data Deletion**       | I enable **versioning** to retain deleted object versions and apply **MFA Delete** for an extra security layer. This ensures accidental or malicious deletes can be reversed.                                          |
| **Detecting and Blocking Public Access**      | I use **Amazon S3 Block Public Access** settings to override ACLs or policies that may unintentionally allow public access — essential for security compliance.                                                        |
| **Auditing Who Accessed What**                | I enable **S3 server access logs** and **AWS CloudTrail** to audit object-level activity like uploads, deletes, and downloads for all users and services.                                                              |
| **Ensuring Compliance for Sensitive Data**    | I enforce **encryption-at-rest** using **SSE-KMS** with CMK and use **bucket policies** to deny uploads without encryption. I also integrate **Macie** for PII discovery.                                              |
| **Serving Different Versions of a File**      | I use **object versioning** to maintain and serve different versions. In metadata, I tag versions appropriately and access them using **version IDs** when needed.                                                     |
| **Invalidating Cached Files After Update**    | When serving through **CloudFront**, I issue a **cache invalidation request** after updating a file in S3 to ensure users receive the latest version immediately.                                                      |
| **Bucket Policy**         | A **bucket policy** is a **resource-based JSON policy** that defines access to an entire S3 bucket or specific objects. It allows fine-grained control like **allowing/restricting access for specific IAM users, services, IPs, or VPCs**.                    |
| **IAM vs Bucket Policy**  | **IAM policies** are **identity-based** (attached to users/roles), while **bucket policies** are **resource-based** (attached to buckets). IAM controls **who can act**, bucket policy controls **what actions can be performed on a specific bucket/object**. |
| **Controlled Actions**    | You can control **S3 actions** like `s3:GetObject`, `s3:PutObject`, `s3:ListBucket`, etc., using either **IAM policies or bucket policies**, ensuring **least-privilege access**.                                                                              |
| **Restrict by IP**        | Use **bucket policies** with `Condition` key like `aws:SourceIp` to **restrict access to a set of IP addresses**, often used for enterprise apps or VPC-limited traffic.                                                                                       |
| **Public Access**         | Public access can be granted via **bucket policies or object ACLs**. However, it's **highly discouraged** unless the use case is static website hosting or public datasets.                                                                                    |
| **Block Public Access**   | **Block Public Access settings override all public permissions**, ensuring that no object or bucket can be made public accidentally — crucial for **security and compliance**.                                                                                 |
| **ACLs**                  | **Access Control Lists (ACLs)** predate bucket policies and allow **granting access to specific AWS accounts**. They're less flexible and harder to audit than policies.                                                                                       |
| **ACLs vs Policies**      | Use **ACLs** only for **legacy systems** or **cross-account sharing**, otherwise prefer **bucket policies or IAM** for better control, scalability, and auditing.                                                                                              |
| **S3 CORS**               | **Cross-Origin Resource Sharing** config is needed if your frontend (on another domain) accesses S3 directly (e.g., `fetch()` from JS). Define allowed origins, methods, and headers.                                                                          |
| **Encryption**            | Supports **SSE-S3 (default)**, **SSE-KMS (with CMK)**, and **SSE-C (client-provided key)**. Use **SSE-KMS** for compliance, auditing, and fine-grained key access control.                                                                                     |
| **Pre-Signed URLs**       | Useful for **secure, time-limited uploads/downloads**. Only someone with the **signed URL and before expiry** can access the object. Common in file-sharing and media uploads.                                                                                 |
| **Lifecycle Policies**    | Automate **transition of objects** between storage classes (e.g., Standard → Glacier) or **deletion**. Helps in **cost optimization** and **data retention compliance**.                                                                                       |
| **Storage Classes**       | Choose based on access frequency: **Standard**, **IA**, **One Zone-IA**, **Glacier**, **Deep Archive**, and **Intelligent-Tiering**. Each varies in **cost vs latency trade-offs**.                                                                            |
| **S3 Security**           | Combine **bucket policies**, **IAM roles**, **MFA delete**, **encryption**, and **Block Public Access** to implement **defense-in-depth**.                                                                                                                     |
| **Versioning**            | Maintains **history of object versions**, useful for recovery, auditing, or undoing accidental deletions. Must be **enabled explicitly**.                                                                                                                      |
| **Large Uploads**         | Use **Multipart Uploads** for files >100 MB. Improves **upload speed**, **resilience to failure**, and allows **parallel uploads**.                                                                                                                            |
| **Transfer Acceleration** | Speeds up global uploads via **CloudFront edge locations**. Use when users are uploading from **remote locations** with high latency.                                                                                                                          |
...
| **Services**          | • [Deployment & Environment Management](#-deployment--environment-management) • [CI/CD and DevOps](#️-cicd-and-devops) • [Monitoring & Logging](#-monitoring--logging) • [Scalability & Serverless Design](#-scalability--serverless-design) • [Data Engineering with Node.js](#-data-engineering-with-nodejs) • [Cosmos DB Integration](#️-cosmos-db-integration) • [Resilience & Architecture](#-resilience--architecture) • [Security & Compliance](#-security--compliance)
