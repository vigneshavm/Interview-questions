...
- A Query Execution Plan explains how the database engine executes a SQL query internally
- It shows details like which indexes are used, join types, and the estimated cost of each operation.
- I usually check it using the `EXPLAIN` or `EXPLAIN ANALYZE` command to identify slow parts of the query
— for example, full table scans or inefficient joins — and then optimize indexes or rewrite queries accordingly.

A **Query Execution Plan (QEP)** shows **how the database executes your SQL query step by step** — which indexes it uses, what join methods it applies, and how it retrieves and filters data internally.
It’s generated by the **query optimizer** inside the database engine.

Think of it as a **blueprint** that explains *how* the DBMS will fetch your result, not *just what* it will fetch.

 **How It Works**

When you run a query:

1. The **parser** validates your SQL syntax.
2. The **optimizer** evaluates multiple possible paths (e.g., using different indexes, join orders).
3. The **execution plan** is chosen — the one with the lowest estimated cost.
4. The plan shows **access paths**, **join order**, **filters**, and **estimated costs**.

 **Key Components in a Plan**

| Term                   | Meaning                                                    |
| ---------------------- | ---------------------------------------------------------- |
| **Table / Relation**   | Which tables are accessed                                  |
| **Access Type**        | How rows are retrieved (Full Scan, Index Scan, etc.)       |
| **Join Type**          | How tables are joined (Nested Loop, Hash Join, Merge Join) |
| **Filter / Predicate** | Conditions applied (`WHERE`, `JOIN ON`)                    |
| **Rows / Cost**        | Estimated number of rows processed and cost of operation   |
...
| -- | ----------- | ------ | ---- | --------------- | ---- | --------------------- |
| 1  | SIMPLE      | orders | ref  | idx_customer_id | 5    | Using index condition |

✅ **Interpretation**:

* It’s using the index `idx_customer_id`, not scanning the whole table.
* Only about 5 rows expected → efficient query.
